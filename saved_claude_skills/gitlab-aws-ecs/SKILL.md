---
name: gitlab-aws-ecs
description: Set up a GitLab CI/CD pipeline for deploying a monorepo (FastAPI backend + React/Vite frontend) to AWS ECS Express Mode via ECR
trigger: /gitlab-aws-ecs
---

# /gitlab-aws-ecs

Set up or repair a GitLab CI/CD → AWS ECS Express Mode deployment for a project with a FastAPI backend and React/Vite frontend.
...
Create a user (e.g. `gitlab-ci-clockgame`) with no console access. Attach:
...
aws ecs create-express-gateway-service \
  --service-name clockgame-backend \
  --primary-container '{"image":"ACCOUNT.dkr.ecr.REGION.amazonaws.com/clockgame-backend:latest","containerPort":8080,"environment":[{"name":"CORS_ORIGINS","value":"*"}]}' \
...
Then frontend (with CORS set to the frontend URL after creating it):
```bash
aws ecs create-express-gateway-service \
  --service-name clockgame-frontend \
  --primary-container '{"image":"ACCOUNT.dkr.ecr.REGION.amazonaws.com/clockgame-frontend:latest","containerPort":8080}' \
  --execution-role-arn arn:aws:iam::ACCOUNT:role/ecsTaskExecutionRole \
  --infrastructure-role-arn arn:aws:iam::ACCOUNT:role/ecsInfrastructureRoleForExpressServices \
  --region REGION
```
...
build-backend-image:
  ...
  script:
    - REPO=$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/clockgame-backend
    - docker build -t $REPO:$CI_COMMIT_SHA -t $REPO:latest ./backend
    - docker push $REPO:$CI_COMMIT_SHA
    - docker push $REPO:latest
  rules:
    - if: $CI_COMMIT_BRANCH == "feature/aws-apprunner"
...
build-frontend-image:
  ...
  script:
    - REPO=$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/clockgame-frontend
...
deploy-backend-aws:
  ...
  script:
    - aws ecs update-express-gateway-service
        ...
        --primary-container "{\"image\":\"$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/clockgame-backend:$CI_COMMIT_SHA\"}"
        --region $AWS_REGION
  rules:
    - if: $CI_COMMIT_BRANCH == "feature/aws-apprunner"
...
deploy-frontend-aws:
  ...
  script:
    - aws ecs update-express-gateway-service
        ...
        --primary-container "{\"image\":\"$AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/clockgame-frontend:$CI_COMMIT_SHA\"}"
        --region $AWS_REGION
  rules:
    - if: $CI_COMMIT_BRANCH == "feature/aws-apprunner"
```
