...
Set up or audit a GitHub Actions → Railway deployment for a project with a FastAPI backend and React/Vite frontend.
...
Ask the user for:
- Exact Railway service names (e.g. `clockgame-backend`, `clockgame-frontend`) — must match Railway dashboard exactly
